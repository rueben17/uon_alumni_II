...
def _profile_form_data(**overrides):
    ...
    mandatory. phone_alt and email were dropped from that allow-list
    (main, 2026-08-27, "Make Alt Email and Alt Phone required") -- both
    included here as real defaults so tests not specifically exercising
    them don't spuriously fail on an unrelated required-field error.
    """
    data = {
        "surname": "Kamau",
        "first_name": "Wanjiku",
...
class AlumniProfileFormPhoneTests(TestCase):
    ...
    def test_alternate_phone_is_required(self):
        """main, 2026-08-27: phone_alt was dropped from optional_fields --
        blank now refuses, not silently accepted."""
