...
## 7. Addendum — 2026-08-21

Two production reports investigated the night after this audit, same evidence standard (real `journalctl -u uon_alumni` log lines / live DB queries, not static analysis).

1. **`Internal Server Error: /2005/home/images/add/`, one occurrence, 2026-08-21 04:29:27 — same category as item 10 above, NOT independently fixed.** Full traceback confirms `psycopg2.errors.InvalidCursorName: cursor "_django_curs_..." does not exist` while Django's admin rendered a ForeignKey `<select>` on the "Add Gallery Image" form — a named server-side cursor was orphaned mid-request, consistent with a Neon connection-pool handoff during a low-traffic hour. `CONN_HEALTH_CHECKS=True` doesn't catch this specific failure mode (the connection object still looks alive; a previously-opened server-side cursor on it doesn't survive the handoff). **Status: observed, not fixed** — no code or config change was made for this; it remains item 10's existing "monitor rather than act" posture unless it starts recurring at meaningfully higher frequency.
2. **"Program Area images are empty" on the live homepage, reported 2026-08-21 — CONFIRMED and RESOLVED.** Root cause: the same night's deploy added six dedicated `Banner.*_image` fields (migration `0038_banner_profile_update_card_image_and_more`) and the corresponding image *files* were committed to git and correctly reached the production filesystem via the normal `git reset --hard origin/main` deploy step — but `deploy.sh` deliberately never seeds field *values* (its own comment: "upload those per ProgramArea row via the production admin"), so production's single `Banner` row (pk=1) still had all six fields `None` after deploy. Fix applied directly against the production DB (`manage.py shell` over SSH): pointed the row's six fields at the already-present files (paths verified identical to what the local dev row used, and verified present on the production filesystem via `find` before writing). Verified live afterward: the file URLs return `200` and all six paths now appear in the rendered homepage HTML. The two homepage hero-card fields (`profile_update_card_image`, `volunteer_card_image`) are still unset on both local and production — no photos have been uploaded for those anywhere yet; they fall back to existing generic default banner images rather than rendering broken, so this is a content gap, not an error.
